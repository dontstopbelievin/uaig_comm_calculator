...
export default class ProviderHeat extends React.Component {
  ...
          var data = JSON.parse(xhr.responseText);
          ...
          var acc_forms_list = data.filter(function(obj) { return obj.ApzHeatStatus === 1; });
          this.setState({acceptedForms: acc_forms_list});
          // filter the list to get the declined apzForms
          var dec_forms_list = data.filter(function(obj) { return obj.ApzHeatStatus === 0; });
...
      if (xhr.status === 200) {
        ...
        this.setState({ hGen: data.HeatGeneral }); 
        this.setState({ hMain: data.HeatMain }); 
        this.setState({ hVen: data.HeatVentilation }); 
        this.setState({ hWater: data.HeatWater }); 
        this.setState({ hTech: data.HeatTech });
        this.setState({ hDist: data.HeatDistribution });
        this.setState({ hSav: data.HeatSaving });
