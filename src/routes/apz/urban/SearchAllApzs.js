...
export default class UrbanSearchAllApzs extends React.Component {
  constructor(props) {
    super(props);
  ...
  getAll(page) {
    ...
    xhr.open("get", window.url + "api/apz/urban/search?page=" + page + "&" + data, true);
  ...
  search(page) {
    ...
    this.props.history.push('/panel/urban/apz/search/' + page + '?' + data);
  }
              ...
              <nav className="pagination_block">
                <ul className="pagination justify-content-center">
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/urban/apz/search/1' + search}>В начало</Link>
                  </li>

                  {this.state.pageNumbers.map(function(num, index) {
                    return(
                      <li key={index} className={'page-item ' + (page == num ? 'active' : '')}>
                        <Link className="page-link" to={'/panel/urban/apz/search/' + num + search}>{num}</Link>
                      </li>
                      );
                    }.bind(this))
                  }
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/urban/apz/search/' + this.state.response.last_page + search}>В конец</Link>
                  </li>
                </ul>
              </nav>
            }
