...
export default class AllApzs extends React.Component {
  constructor(props) {
    ...
    this.state = {
      loaderHidden: false,
      isPerformer: roles != null ? (roles.indexOf('PerformerGas') !== -1) : null,
      response: null,
      data: null,
      pageNumbers: []
    };
                ...
                {apzs.map(function(apz, index) {
                  return(
                    <tr key={index}>
                      ...
                      <td>
                        <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/gas-provider/apz/show/' + apz.id}><i className="glyphicon glyphicon-eye-open mr-2"></i> Просмотр</Link>
                      </td>
                    </tr>
                    );
                  }.bind(this))
