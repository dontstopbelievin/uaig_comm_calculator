          ...
          {this.state.loaderHidden &&
            <div>
              <ul className="nav nav-tabs mb-2 pull-right">
                ...
                <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => status === 'ss_awaiting'} to="/panel/state_services/apz/status/ss_awaiting/1" replace>В процессе</NavLink></li>
                  ...
                  {apzs.map(function(apz, index) {
                    return(
                      <tr key={index}>
                        ...
                        <td>
                          <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/state_services/apz/show/' + apz.id}><i className="glyphicon glyphicon-eye-open mr-2"></i> Просмотр</Link>
                        </td>
                      </tr>
                      );
                    }.bind(this))
