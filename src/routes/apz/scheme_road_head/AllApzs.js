...
export default class AllApzs extends React.Component {
  ...
  getApzs(status = null, page = null) {
    ...
    xhr.open("get", window.url + "api/apz/schemeroadhead/all/" + status + '?page=' + page, true);
        ...
        {this.state.loaderHidden &&
          <div>
            ...
            <tr><td>
              ...
              <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => status === 'active'} to="/panel/schemeroadhead/apz/status/active/1" replace>Активные</NavLink></li>
              <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => status === 'accepted'} to="/panel/schemeroadhead/apz/status/accepted/1" replace>Отработанные</NavLink></li>
              </ul>
            </td></tr></tbody>
            </table>
