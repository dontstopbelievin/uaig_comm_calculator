...
export default class ShowApz extends React.Component {
    ...
    getApzInfo() {
      ...
      xhr.open("get", window.url + "api/apz/generalplanscheme/detail/" + id, true);
    ...
    getTokenXml(alias) {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("get", window.url + 'api/apz/generalplanscheme/get_xml/' + this.state.apz.id, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function() {
    ...
    signXmlBack(result) {
      if (result['errorCode'] === "NONE") {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("post", window.url + 'api/apz/generalplanscheme/save_xml/' + this.state.apz.id, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
        xhr.onload = function() {
          if (xhr.status === 200) {
    ...
    acceptDeclineApzForm(apzId, status, comment, direct) {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("post", window.url + "api/apz/generalplanscheme/status/" + apzId, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function () {
        if (xhr.status === 200) {
    ...
    showSignBtns(){
        if (!this.state.description || this.state.description.trim() == '' || !this.state.schemeFile) {
          alert('Напишите комментарий и закрепите файл!');
          return false;
        }
                    ...
                    <tbody>
                      ...
                      {this.state.schemeFile &&
                        <tr>
                          ...
                          <td><a className="text-info pointer" data-category="6" onClick={this.downloadFile.bind(this, this.state.schemeFile.id, 6)}>Скачать</a>
                  ...
                  <div className="form-group">
                    <div className="file_container">
                      <div className="col-md-12">
                        <div className="progress mb-2" data-category="38" style={{height: '20px', display: 'none'}}>
                          <div className="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style={{width: '0%'}} aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                      </div>
                      {this.state.schemeFile &&
                        <div className="file_block mb-2">
                          <div>
                            {this.state.schemeFile.name}
                            <a className="pointer" onClick={(e) => this.setState({schemeFile: false}) }>×</a>
                          </div>
                        </div>
                      }
                      <div className="file_buttons btn-group btn-group-justified d-table mt-0">
                        <label><h6>Ситуационная схема</h6></label>
                        <label htmlFor="schemeFile" className="btn btn-success" style={{marginLeft: '5px'}}>Загрузить</label>
                        <input type="file" id="schemeFile" name="schemeFile" className="form-control" onChange={this.uploadFile.bind(this, 38)} style={{display: 'none'}} />
                      </div>
                      <span className="help-block text-muted">файл в формате doc, docx, pdf, jpg, png, tiff</span>
                    </div>
                  </div>
