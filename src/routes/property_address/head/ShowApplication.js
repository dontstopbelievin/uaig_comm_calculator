...
export default class ShowApplication extends React.Component {
    ...
    getApplications() {
        ...
        xhr.open("get", window.url + "api/property_address/head/detail/" + id, true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                this.setState({propertyaddress: data});
                ...
                /*if (data.property_address_head_responses && data.property_address_head_responses.files) {
                  this.setState({headResponseFile: data.property_address_head_responses.files.filter(function(obj) { return obj.category_id === 11 || obj.category_id === 12 })[0]});
                }*/

                if (data.status_id === 4 && !data.property_address_head_responses) {
                    this.setState({showButtons: true});
                }

                if (data.status_id === 4 && data.property_address_head_responses && data.files.filter(function(obj) { return obj.category_id === 19})[0] == null) {
                    this.setState({showButtons: true});
                }

                if (data.property_address_head_responses && data.property_address_head_responses && data.files.filter(function(obj) { return obj.category_id === 19})[0] != null) {
                    this.setState({isSigned: true});
                    this.setState({xmlFile: data.files.filter(function(obj) { return obj.category_id === 19})[0]});
                    this.setState({headResponse: data.property_address_head_responses.response});
                }

                if (data.property_address_head_responses && data.files.filter(function(obj) { return obj.category_id === 19})[0] != null && data.status_id === 4) {
    ...
    beforeSign(){
      this.saveApplicationForm(this.state.propertyaddress.id, this.state.lastDecisionIsMO ? false : true, "");
    }
    ...
    saveApplicationForm(propertyaddressId, status, comment) {
        var token = sessionStorage.getItem('tokenInfo');
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("post", window.url + "api/property_address/head/saveHead/" + propertyaddressId, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        xhr.onload = function () {
            if (xhr.status === 200) {
                var data = JSON.parse(xhr.responseText);
                ...
                if(this.state.callSaveFromSend){
                    ...
                    this.acceptDeclineForm(propertyaddressId, status, comment);
                } else {
    ...
    returnApplicationForm(propertyaddressId) {
        var token = sessionStorage.getItem('tokenInfo');
        var formData = new FormData();
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("post", window.url + "api/property_address/head/return/" + propertyaddressId, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        xhr.onload = function () {
            if (xhr.status === 200) {
                this.setState({ showButtons: false });
    ...
    acceptDeclineForm(propertyaddressId, status, comment) {
        if(this.state.headResponse === null){
            this.setState({callSaveFromSend: true});
            this.saveApplicationForm(propertyaddressId, status, comment);
            return true;
        }
        ...
        var xhr = new XMLHttpRequest();
        xhr.open("post", window.url + "api/property_address/head/status/" + propertyaddressId, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        xhr.onload = function () {
            if (xhr.status === 200) {
                //var data = JSON.parse(xhr.responseText);
        ...
        return (
            <div>
              {this.state.loaderHidden &&
                <div>
                  <AllInfo toggleMap={this.toggleMap.bind(this, true)} propertyaddress={this.state.propertyaddress} personalIdFile={this.state.personalIdFile}
                 ...
                 <Answers propertyaddress_id={this.state.propertyaddress.id} propertyaddress_status={this.state.propertyaddress.status_id}
                  ...
                  <div className="btn-group" role="group" aria-label="acceptOrDecline" style={{margin: 'auto', marginTop: '20px', display: 'table'}}>
                    ...
                    <EcpSign beforeSign={this.beforeSign.bind(this)} ecpSignSuccess={this.ecpSignSuccess.bind(this)} hideSignBtns={this.hideSignBtns.bind(this)} rolename="head" id={this.state.propertyaddress.id} serviceName='property_address'/>
                    }
                            ...
                            <div className="modal-dialog" role="document">
                                <div className="modal-content">
                                    ...
                                    <div className="modal-footer">
                                        <button type="button" className="btn btn-raised btn-success" style={{marginRight: '5px'}} onClick={this.returnApplicationForm.bind(this, this.state.propertyaddress.id)}>Отправить</button>
...

                    {
                      ...
                      <button type="button" className="btn btn-raised btn-success" onClick={this.acceptDeclineForm.bind(this, this.state.propertyaddress.id, this.state.lastDecisionIsMO ? false : true, "")}>Отправить заявителю</button>
                    }
                  </div>
