...
export default class AllApplications extends React.Component {
    ...
    getApplications(status = null, page = null) {
        ...
        var token = sessionStorage.getItem('tokenInfo');
        var xhr = new XMLHttpRequest();
        xhr.open("get", window.url + "api/property_address/head/all/" + status + '/' + this.state.current_head + '?page=' + page, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
        xhr.onload = function () {
            if (xhr.status === 200) {
            ...
            <div>
                ...
                <div>
                    ...
                    <ul className="nav nav-tabs mb-2 pull-right">
                        <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'active'} to="/panel/head/propertyaddress/status/active/1" replace>Активные</NavLink></li>
                        <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'inproccess'} to="/panel/head/propertyaddress/status/inproccess/1" replace>В процессе</NavLink></li>
                        <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'accepted'} to="/panel/head/propertyaddress/status/accepted/1" replace>Принятые</NavLink></li>
                        <li className="nav-item"><NavLink activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'declined'} to="/panel/head/propertyaddress/status/declined/1" replace>Отказанные</NavLink></li>
                    </ul>
                        ...
                        {this.state.data.map(function(application, index) {
                            return(
                                <tr key={index}>
                                    ...
                                    <td>
                                        <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/head/propertyaddress/show/' + application.id}><i className="glyphicon glyphicon-eye-open mr-2"></i> Просмотр</Link>
                                    </td>
                                </tr>
                            );
                        }.bind(this))
                    ...
                    <nav className="pagination_block">
                        <ul className="pagination justify-content-center">
                            <li className="page-item">
                                <Link className="page-link" to={'/panel/head/propertyaddress/status/' + this.props.match.params.status + '/1'}>В начало</Link>
                            </li>

                            {this.state.pageNumbers.map(function(num, index) {
                                return(
                                    <li key={index} className={'page-item ' + (this.props.match.params.page === num ? 'active' : '')}>
                                        <Link className="page-link" to={'/panel/head/propertyaddress/status/' + this.props.match.params.status + '/' + num}>{num}</Link>
                                    </li>
                                );
                            })
                            }
                            <li className="page-item">
                                <Link className="page-link" to={'/panel/head/propertyaddress/status/' + this.props.match.params.status + '/' + this.state.response.last_page}>В конец</Link>
                            </li>
                        </ul>
                    </nav>
                    }
