...
export default class AddApplication extends React.Component {
    ...
    getLandInLocalityInfo() {
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          ...
          this.setState({applicant: propertyaddress.applicant ? propertyaddress.applicant : '' });
          this.setState({applicantAddress: propertyaddress.address ? propertyaddress.address : '' });
          this.setState({applicantPhone: propertyaddress.phone ? propertyaddress.phone : '' });
          this.setState({region: propertyaddress.region ? propertyaddress.region : '' });
          this.setState({landAddress: propertyaddress.land_address ? propertyaddress.land_address : '' });
          this.setState({landAddressCoordinates: propertyaddress.land_address_coordinates ? propertyaddress.land_address_coordinates : '' });
          this.setState({hasCoordinates: propertyaddress.land_address_coordinates ? true : false });
          ...
          this.setState({cadastralNumber: propertyaddress.cadastral_number ? propertyaddress.cadastral_number : '' });
          this.setState({status: propertyaddress.status_id? propertyaddress.status_id : '' });
        }
    ...
    saveLandInLocality(publish, elem) {
      ...
      var propertyaddressId = this.props.match.params.id;

      switch(this.state.status){
          case 1://declined
            var link="api/property_address/citizen/save";
            break;
          default:
            var link = propertyaddressId > 0 ? ("api/property_address/citizen/save/" + propertyaddressId) : "api/property_address/citizen/save";
            break;
      }
      ...
      xhr.onload = function() {
        ...
        if (xhr.status === 200) {
          ...
          if (publish) {
            ...
            this.props.history.push('/panel/citizen/property_address/status/active/1');
          } else {
            alert('Заявка успешно сохранена');

            if (!propertyaddressId) {
              this.props.history.push('/panel/citizen/property_address/edit/' + data.id);
            }
          }
        } else {
    ...
    render() {
      return (
        <div className="container" id="propertyaddressFormDiv">
