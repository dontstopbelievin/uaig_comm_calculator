...
export default class Answers extends React.Component {
    ...
    printRegionAnswer(applicationId, progbarId = null) {
      ...
      if (token) {
        var xhr = new XMLHttpRequest();
        xhr.open("get", window.url + "api/print/region/reshapetorelig/" + applicationId, true);
        xhr.setRequestHeader("Authorization", "Bearer " + token);
        var vision = $('.text-info[data-category='+progbarId+']');
        var progressbar = $('.progress[data-category='+progbarId+']');
        vision.css('display', 'none');
    ...
    render() {
      return (
            <div>
              ...
              {(this.props.reshapetorelig_status === 2 || this.props.actChooseLandFile) && !this.props.lastDecisionIsMO &&
                <div>
              ...
              {(this.props.reshapetorelig_status === 1 || this.props.lastDecisionIsMO) &&
                <table className="table table-bordered">
                  <tbody>
                    <tr>
                      <td style={{width: '22%'}}><b>Мотивированный отказ</b></td>
                      <td>
                        <a className="text-info pointer" data-category="14" onClick={this.printRegionAnswer.bind(this, this.props.reshapetorelig_id, 14)}>Скачать</a>
