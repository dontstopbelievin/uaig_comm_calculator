...
export default class AddApplication extends React.Component {
    constructor(props) {
      ...
      this.saveReshapeToRelig = this.saveReshapeToRelig.bind(this);
    ...
    componentWillMount() {
      if (this.props.match.params.id) {
        this.getReshapeToReligInfo();
      }
    }

    getReshapeToReligInfo() {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("get", window.url + "api/reshape_to_relig/citizen/detail/" + id, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function() {
        if (xhr.status === 200) {
          var reshapetorelig = JSON.parse(xhr.responseText);

          this.setState({applicant: reshapetorelig.applicant ? reshapetorelig.applicant : '' });
          this.setState({applicantAddress: reshapetorelig.address ? reshapetorelig.address : '' });
          this.setState({applicantPhone: reshapetorelig.phone ? reshapetorelig.phone : '' });
          this.setState({region: reshapetorelig.region ? reshapetorelig.region : '' });
          this.setState({landAddress: reshapetorelig.land_address ? reshapetorelig.land_address : '' });
          this.setState({landAddressCoordinates: reshapetorelig.land_address_coordinates ? reshapetorelig.land_address_coordinates : '' });
          this.setState({hasCoordinates: reshapetorelig.land_address_coordinates ? true : false });
          this.setState({personalIdFile: reshapetorelig.files.filter(function(obj) { return obj.category_id === 3 })[0]});
          this.setState({landLocationSchemeFile: reshapetorelig.files.filter(function(obj) { return obj.category_id === 42 })[0]});
          this.setState({cadastralNumber: reshapetorelig.cadastral_number ? reshapetorelig.cadastral_number : '' });
          this.setState({status: reshapetorelig.status_id? reshapetorelig.status_id : '' });
        }
    ...
    saveReshapeToRelig(publish, elem) {
      elem.preventDefault();
      ...
      var reshapetoreligId = this.props.match.params.id;

      switch(this.state.status){
          case 1://declined
            var link="api/reshape_to_relig/citizen/save";
            break;
          default:
            var link = reshapetoreligId > 0 ? ("api/reshape_to_relig/citizen/save/" + reshapetoreligId) : "api/reshape_to_relig/citizen/save";
            break;
      }
      ...
      xhr.onload = function() {
        ...
        if (xhr.status === 200) {
          ...
          if (publish) {
            ...
            this.props.history.push('/panel/citizen/reshape_to_relig/status/active/1');
          } else {
            alert('Заявка успешно сохранена');

            if (!reshapetoreligId) {
              this.props.history.push('/panel/citizen/reshape_to_relig/edit/' + data.id);
            }
          }
        } else {
    ...
    render() {
      return (
        <div className="container" id="reshapetoreligFormDiv">
                ...
                <div className="col-8">
                  <div className="tab-content" id="v-pills-tabContent">
                    <div className="tab-pane fade show active" id="tab0" role="tabpanel" aria-labelledby="tab0-link">
                      ...
                      <form id="tab0-form" data-tab="0" onSubmit={this.saveReshapeToRelig.bind(this, false)}>
                        <div className="row">
                      ...
                      <button onClick={this.saveReshapeToRelig.bind(this, true)} className="btn btn-outline-success">Отправить заявку</button>
                    </div>
                    <div className="tab-pane fade" id="tab1" role="tabpanel" aria-labelledby="tab1-link">
                        <div style={{marginBottom:'16px'}}>
                          <span style={{color:'red'}}>*</span><span style={{color:'#888'}}> - поля обязательные к заполнению</span>
                        </div>
                        <form id="tab1-form" data-tab="1" onSubmit={this.saveReshapeToRelig.bind(this, false)}>
                            <div className="row">
                              <div className="col-md-7">
