...
export default class AddApplication extends React.Component {
    constructor(props) {
      ...
      this.saveReligBuilding = this.saveReligBuilding.bind(this);
    ...
    componentWillMount() {
      if (this.props.match.params.id) {
        this.getReligBuildingInfo();
      }
    }

    getReligBuildingInfo() {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("get", window.url + "api/relig_building/citizen/detail/" + id, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function() {
        if (xhr.status === 200) {
          var religbuilding = JSON.parse(xhr.responseText);

          this.setState({applicant: religbuilding.applicant ? religbuilding.applicant : '' });
          this.setState({applicantAddress: religbuilding.address ? religbuilding.address : '' });
          this.setState({applicantPhone: religbuilding.phone ? religbuilding.phone : '' });
          this.setState({region: religbuilding.region ? religbuilding.region : '' });
          this.setState({landAddress: religbuilding.land_address ? religbuilding.land_address : '' });
          this.setState({landAddressCoordinates: religbuilding.land_address_coordinates ? religbuilding.land_address_coordinates : '' });
          this.setState({hasCoordinates: religbuilding.land_address_coordinates ? true : false });
          this.setState({personalIdFile: religbuilding.files.filter(function(obj) { return obj.category_id === 3 })[0]});
          this.setState({landLocationSchemeFile: religbuilding.files.filter(function(obj) { return obj.category_id === 42 })[0]});
          this.setState({cadastralNumber: religbuilding.cadastral_number ? religbuilding.cadastral_number : '' });
          this.setState({status: religbuilding.status_id? religbuilding.status_id : '' });
        }
    ...
    saveReligBuilding(publish, elem) {
      elem.preventDefault();
      ...
      var religbuildingId = this.props.match.params.id;

      switch(this.state.status){
          case 1://declined
            var link="api/relig_building/citizen/save";
            break;
          default:
            var link = religbuildingId > 0 ? ("api/relig_building/citizen/save/" + religbuildingId) : "api/relig_building/citizen/save";
            break;
      }
      ...
      xhr.onload = function() {
        ...
        if (xhr.status === 200) {
          ...
          if (publish) {
            ...
            this.props.history.push('/panel/citizen/relig_building/status/active/1');
          } else {
            alert('Заявка успешно сохранена');

            if (!religbuildingId) {
              this.props.history.push('/panel/citizen/relig_building/edit/' + data.id);
            }
          }
        } else {
    ...
    render() {
      return (
        <div className="container" id="religbuildingFormDiv">
                ...
                <div className="col-8">
                  <div className="tab-content" id="v-pills-tabContent">
                    <div className="tab-pane fade show active" id="tab0" role="tabpanel" aria-labelledby="tab0-link">
                      ...
                      <form id="tab0-form" data-tab="0" onSubmit={this.saveReligBuilding.bind(this, false)}>
                        <div className="row">
                      ...
                      <button onClick={this.saveReligBuilding.bind(this, true)} className="btn btn-outline-success">Отправить заявку</button>
                    </div>
                    <div className="tab-pane fade" id="tab1" role="tabpanel" aria-labelledby="tab1-link">
                        <div style={{marginBottom:'16px'}}>
                          <span style={{color:'red'}}>*</span><span style={{color:'#888'}}> - поля обязательные к заполнению</span>
                        </div>
                        <form id="tab1-form" data-tab="1" onSubmit={this.saveReligBuilding.bind(this, false)}>
                            <div className="row">
                              <div className="col-md-7">
