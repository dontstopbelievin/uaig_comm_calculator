...
export default class AllReligBuildingHistory extends React.Component {
  constructor(props) {
    super(props);
  ...
  getAll(user_id = null, page = null) {
    ...
    xhr.open("get", window.url + "api/relig_building/all_history/"+user_id+"?page=" + page + "&" + data, true);
  ...
  search(user_id, page) {
    ...
    this.props.history.push('/panel/religbuilding/all_history/'+ user_id +'/'+ page + '?' + data);
  }
                ...
                {this.state.response.data && this.state.response.data.map(function(application, index) {
                  return(
                    <tr key={index}>
                      ...
                      <td>{application.religbuilding_status.name}</td>
                      <td>
                        <Link className="btn btn-outline-info" to={'/panel/'+this.state.rolename+'/religbuilding/show/' + application.id}><i className="glyphicon glyphicon-eye-open mr-2"></i> Просмотр</Link>
                      </td>
                    </tr>
                    );
                  }.bind(this))
              ...
              <nav className="pagination_block">
                <ul className="pagination justify-content-center">
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/religbuilding/all_history/'+ this.props.match.params.user_id + '/1' + this.props.location.search}>В начало</Link>
                  </li>

                  {this.state.pageNumbers.map(function(num, index) {
                    return(
                      <li key={index} className={'page-item ' + (this.props.match.params.page === num ? 'active' : '')}>
                        <Link className="page-link" to={'/panel/religbuilding/all_history/' + this.props.match.params.user_id+ '/' + num + this.props.location.search}>{num}</Link>
                      </li>
                      );
                    })
                  }
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/religbuilding/all_history/' + this.props.match.params.user_id + '/' + this.state.response.last_page + this.props.location.search}>В конец</Link>
                  </li>
                </ul>
              </nav>
            }
