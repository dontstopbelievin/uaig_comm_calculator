...
export default class ShowApplication extends React.Component {
    ...
    getApplications() {
      ...
      var token = sessionStorage.getItem('tokenInfo');
      var xhr = new XMLHttpRequest();
      xhr.open("get", window.url + "api/relig_building/region/detail/" + id, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function() {
        if (xhr.status === 200) {
          var response = JSON.parse(xhr.responseText).religbuilding;
          var templates = JSON.parse(xhr.responseText).templates;
          this.setState({religbuilding: response});
    ...
    acceptDeclineForm(applicationId, status, comment) {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("post", window.url + "api/relig_building/region/status/" + applicationId, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function () {
        if (xhr.status === 200) {
        ...
        <div>
          {this.state.loaderHidden &&
            <div>
              ...
              {this.state.showMap && <ShowMap coordinates={this.state.religbuilding.project_address_coordinates} mapId={"b5a3c97bd18442c1949ba5aefc4c1835"}/>}

              <button className="btn btn-raised btn-info" onClick={this.toggleMap.bind(this, !this.state.showMap)} style={{margin: '20px auto 10px'}}>
                {this.state.showMapText}
              </button>

              <Answers backFromHead={this.state.backFromHead} religbuilding_id={this.state.religbuilding.id} lastDecisionIsMO={this.state.lastDecisionIsMO}
                ...
                <div className="btn-group" role="group" aria-label="acceptOrDecline" style={{margin: 'auto', marginTop: '20px', display: 'table'}}>
                  {!this.state.needSign ?
                    ...
                    :
                      <div>
                      { !this.state.xmlFile ?
                          <EcpSign ecpSignSuccess={this.ecpSignSuccess.bind(this)} hideSignBtns={this.hideSignBtns.bind(this)} rolename="region" id={this.state.religbuilding.id} serviceName='relig_building'/>
                        :
                          <div style={{paddingLeft:'5px', fontSize: '18px', textAlign:'center'}}>
                            ...
                            <div>
                              <button type="button" className="btn btn-raised btn-success" onClick={this.acceptDeclineForm.bind(this, this.state.religbuilding.id, true, "")}>Отправить главному архитектору</button>
                            </div>
                          </div>
                      }
                    </div>
                  ...
                  <MotivationalReject templates={this.state.templates} theme={this.state.theme} comment={this.state.comment} head_id={this.state.head_id}
                    acceptDeclineForm={this.acceptDeclineForm.bind(this, this.state.religbuilding.id, false, this.state.comment)} head_ids={this.state.head_ids}
