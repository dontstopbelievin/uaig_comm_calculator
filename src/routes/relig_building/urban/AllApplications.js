...
export default class AllApplications extends React.Component {
  ...
  getApplications(status = null, page = null) {
    ...
    xhr.open("get", window.url + "api/relig_building/region/all/" + status + '?page=' + page, true);
          ...
          <div>
            ...
            <tr><td>
              ...
              <ul className="nav nav-tabs mb-2 pull-right">
                <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'active'} to="/panel/urban/religbuilding/status/active/1" replace>Активные</NavLink></li>
                <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'accepted'} to="/panel/urban/religbuilding/status/accepted/1" replace>Принятые</NavLink></li>
                <li className="nav-item"><NavLink exact activeClassName="nav-link active" className="nav-link" activeStyle={{color:"black"}} isActive={(match, location) => this.props.match.params.status === 'declined'} to="/panel/urban/religbuilding/status/declined/1" replace>Отказанные</NavLink></li>
              </ul>
            </td></tr></tbody>
            </table>
                ...
                {this.state.data && this.state.data.map(function(application, index) {
                  return(
                    <tr key={index}>
                      ...
                      <td>
                        <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/urban/religbuilding/show/' + application.id}><i className="glyphicon glyphicon-eye-open mr-2"></i> Просмотр</Link>
                      </td>
                    </tr>
                    );
                  }.bind(this))
              ...
              <nav className="pagination_block">
                <ul className="pagination justify-content-center">
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/urban/religbuilding/status/' + this.props.match.params.status + '/1'}>В начало</Link>
                  </li>

                  {this.state.pageNumbers.map(function(num, index) {
                    return(
                      <li key={index} className={'page-item ' + (this.props.match.params.page === num ? 'active' : '')}>
                        <Link className="page-link" to={'/panel/urban/religbuilding/status/' + this.props.match.params.status + '/' + num}>{num}</Link>
                      </li>
                      );
                    })
                  }
                  <li className="page-item">
                    <Link className="page-link" to={'/panel/urban/religbuilding/status/' + this.props.match.params.status + '/' + this.state.response.last_page}>В конец</Link>
                  </li>
                </ul>
              </nav>
            }
