          ...
          {this.state.loaderHidden &&
            <div>
              ...
              <table className="table allapzs_fonts">
                ...
                <tbody>
                  {this.state.response.data.map(function(applications, index) {
                    return(
                      <tr key={index}>
                        ...
                        <td>
                          <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/citizen/landinlocality/' + (applications.status_id === 5 ? 'edit/' : 'show/') + applications.id}><i className="glyphicon glyphicon-eye-open small"></i> Просмотр</Link>
                          {applications.status_id === 1 &&
                             <Link className="btn btn-outline-info btn-sm allapz_btn" to={'/panel/citizen/landinlocality/edit/'+ applications.id}><i className="glyphicon glyphicon-eye-open small"></i>Переотправить</Link>
                          }
                        </td>
                      </tr>
                      );
                ...
                <nav className="pagination_block">
                  <ul className="pagination justify-content-center">
                    <li className="page-item">
                      <Link className="page-link" to={'/panel/citizen/landinlocality/status/' + this.props.match.params.status + '/1'}>В начало</Link>
                    </li>

                    {this.state.pageNumbers.map(function(num, index) {
                      return(
                        <li key={index} className={'page-item ' + (this.props.match.params.page === num ? 'active' : '')}>
                          <Link className="page-link" to={'/panel/citizen/landinlocality/status/' + this.props.match.params.status + '/' + num}>{num}</Link>
                        </li>
                        );
                      })
                    }
                    <li className="page-item">
                      <Link className="page-link" to={'/panel/citizen/landinlocality/status/' + this.props.match.params.status + '/' + this.state.response.last_page}>В конец</Link>
                    </li>
                  </ul>
                </nav>
              }
