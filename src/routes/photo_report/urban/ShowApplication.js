...
export default class ShowApplication extends React.Component {
    ...
    getApplications() {
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          var response = JSON.parse(xhr.responseText).photoreport;
          var templates = JSON.parse(xhr.responseText).templates;
          this.setState({photoreport: response});
        ...
        <div>
          {this.state.loaderHidden &&
            <div>
              ...
              {this.state.showMap && <ShowMap coordinates={this.state.photoreport.project_address_coordinates} mapId={"b5a3c97bd18442c1949ba5aefc4c1835"}/>}

              <button className="btn btn-raised btn-info" onClick={this.toggleMap.bind(this, !this.state.showMap)} style={{margin: '20px auto 10px'}}>
                {this.state.showMapText}
              </button>

              <Answers backFromHead={this.state.backFromHead} photoreport_id={this.state.photoreport.id} lastDecisionIsMO={this.state.lastDecisionIsMO}
              ...
              <div className={this.state.showButtons ? '' : 'invisible'}>
                <div className="btn-group" role="group" aria-label="acceptOrDecline" style={{margin: 'auto', marginTop: '20px', display: 'table'}}>
                    ...
                    <div style={{margin: 'auto', display: 'table'}}>
                      <div className="file_container">
                        ...
                        <div className="file_buttons btn-group btn-group-justified d-table mt-0">
                          <label><h6>Разрешение на фотоотчет</h6></label>
                      ...
                      { !this.state.xmlFile ?
                        ...
                        :
                          <div style={{paddingLeft:'5px', fontSize: '18px', textAlign:'center'}}>
                            ...
                            <div>
                              <button type="button" className="btn btn-raised btn-success" onClick={this.acceptDeclineForm.bind(this, this.state.photoreport.id, true, "")}>Отправить главному архитектору</button>
                            </div>
                          </div>
                      }
                    </div>
                  ...
                  <MotivationalReject templates={this.state.templates} theme={this.state.theme} comment={this.state.comment} head_id={this.state.head_id}
                    acceptDeclineForm={this.acceptDeclineForm.bind(this, this.state.photoreport.id, false, this.state.comment)} head_ids={this.state.head_ids}
