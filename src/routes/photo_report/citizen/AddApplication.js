...
export default class AddApplication extends React.Component {
    ...
    getLandInLocalityInfo() {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("get", window.url + "api/photo_report/citizen/detail/" + id, true);
      xhr.setRequestHeader("Authorization", "Bearer " + token);
      xhr.setRequestHeader("Content-type", "application/json; charset=UTF-8");
      xhr.onload = function() {
        if (xhr.status === 200) {
          var photoreport = JSON.parse(xhr.responseText);

          this.setState({applicant: photoreport.applicant ? photoreport.applicant : '' });
          this.setState({applicantAddress: photoreport.address ? photoreport.address : '' });
          this.setState({applicantPhone: photoreport.phone ? photoreport.phone : '' });
          this.setState({region: photoreport.region ? photoreport.region : '' });
          this.setState({landAddress: photoreport.land_address ? photoreport.land_address : '' });
          this.setState({landAddressCoordinates: photoreport.land_address_coordinates ? photoreport.land_address_coordinates : '' });
          this.setState({hasCoordinates: photoreport.land_address_coordinates ? true : false });
          this.setState({personalIdFile: photoreport.files.filter(function(obj) { return obj.category_id === 3 })[0]});
          this.setState({photoIdFile: photoreport.files.filter(function(obj) { return obj.category_id === 26 })[0]});
          this.setState({cadastralNumber: photoreport.cadastral_number ? photoreport.cadastral_number : '' });
          this.setState({status: photoreport.status_id? photoreport.status_id : '' });
        }
    ...
    saveLandInLocality(publish, elem) {
      ...
      if (publish) {
        var requiredFields = {
          ...
          photoIdFile: 'Схема расположения земельного участка'
        };
      ...
      var photoreportId = this.props.match.params.id;

      switch(this.state.status){
          case 1://declined
            var link="api/photo_report/citizen/save";
            break;
          default:
            var link = photoreportId > 0 ? ("api/photo_report/citizen/save/" + photoreportId) : "api/photo_report/citizen/save";
            break;
      }
      ...
      xhr.onload = function() {
        ...
        if (xhr.status === 200) {
          ...
          if (publish) {
            ...
            this.props.history.push('/panel/citizen/photo_report/status/active/1');
          } else {
            alert('Заявка успешно сохранена');

            if (!photoreportId) {
              this.props.history.push('/panel/citizen/photo_report/edit/' + data.id);
            }
          }
        } else {
          ...
          setTimeout(function() {
            ...
            switch (category) {
              case 3:
                ...
                break;
              case 26:
                this.setState({photoIdFile: data});
                break;
              default:
            }
    ...
    render() {
      return (
        <div className="container" id="photoreportFormDiv">
                      ...
                      <form id="tab0-form" data-tab="0" onSubmit={this.saveLandInLocality.bind(this, false)}>
                        <div className="row">
                          ...
                          <div className="col-md-5">
                            ...
                            <div className="form-group">
                              <label><span style={{color:'red'}}>*</span> Фото</label>
                              <div className="file_container">
                                <div className="progress mb-2" data-category="26" style={{height: '20px', display: 'none'}}>
                                  <div className="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style={{width: '0%'}} aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                                ...
                                {this.state.photoIdFile &&
                                  <div className="file_block mb-2">
                                    <div>
                                      {this.state.photoIdFile.name}
                                      <a className="pointer" onClick={(e) => this.setState({photoIdFile: false}) }>×</a>
                                    </div>
                                  </div>
                                }

                                <div className="file_buttons btn-group btn-group-justified d-table mt-0">
                                  <label htmlFor="photoIdFile" className="btn btn-success btn-sm" style={{marginRight: '2px'}}>Загрузить</label>
                                  <input type="file" id="photoIdFile" name="photoIdFile" className="form-control" onChange={this.uploadFile.bind(this, 26)} style={{display: 'none'}} />
                                  <label onClick={this.selectFromList.bind(this, 26)} className="btn btn-info btn-sm">Выбрать из списка</label>
                                </div>
                                <span className="help-block text-muted">Изображение в формате jpg</span>
                              </div>
                            </div>
                          </div>
                        </div>
