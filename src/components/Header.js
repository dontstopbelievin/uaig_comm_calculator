...
//import $ from 'jquery';
import { Button, NavLink, UncontrolledDropdown, DropdownToggle, DropdownMenu, DropdownItem } from 'reactstrap';
    ...
    return (
      <div className="header">
        ...
        <div className="container nav-bar" style={{background: '#F8F9FA'}}>
          {/*<Navbar color="light" light expand="md" className="no-border">
  ...
  render() {
    return(
      <div className="row" style={{paddingTop: '0'}}>
        <NavLink to={"/login"} tag={RRNavLink} className="btn btn-white" replace>{e.login}</NavLink>&nbsp;
        <NavLink to={"/register"} tag={RRNavLink} className="btn btn-white" replace>{e.register}</NavLink>
      </div>
    )
  }
}
